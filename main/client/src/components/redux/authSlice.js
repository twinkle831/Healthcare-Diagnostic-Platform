...
export const registerUser = createAsyncThunk('auth/registerUser', async (formData, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://https://https://localhost:4000/api/auth/register', formData, {
        headers: {
...
export const loginUser = createAsyncThunk('auth/loginUser', async (userData, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://https://https://localhost:4000/api/auth/login', userData);
      localStorage.setItem('token', response.data.token);
      return response.data;
    } catch (error) {
